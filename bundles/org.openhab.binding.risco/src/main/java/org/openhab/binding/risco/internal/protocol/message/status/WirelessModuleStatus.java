 ...
 */
@NonNullByDefault
public class WirelessModuleStatus extends RiscoMessage {
    public static final String COMMAND = "WMSTT";

    // @formatter:off
    private final StatusProperty[] properties = {
            new StatusProperty("tamper", "T"),
            new StatusProperty("communication-trouble", "C"),
            new StatusProperty("jamming", "J"),
            new StatusProperty("lost-signal", "L"),
            new StatusProperty("exists", "E") };
    // @formatter:on

    private List<RiscoThing> messageThings = new ArrayList<RiscoThing>();

    public WirelessModuleStatus(int commandId, String commandName, String modifier, String[] commandValues,
    ...
    public static String getReadCommand(int wirelessModuleNumber) {
        return String.format(COMMAND + "%d?", wirelessModuleNumber);
    }
}
