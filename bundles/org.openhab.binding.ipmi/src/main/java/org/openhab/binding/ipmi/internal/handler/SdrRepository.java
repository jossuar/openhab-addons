...
public class SdrRepository {
    ...
    private void addSensorRecord(byte[] buffer) {
        ...
        if (record instanceof CompactSensorRecord) {
            ...
            Sdr sdr = new Sdr(fr.getSensorNumber(), fr.getSensorType(), fr.getName(), fr.getAddressType(),
                    fr.getRateUnit(), fr.getSensorDirection(), fr.getSensorBaseUnit().toString(),
                    fr.getUpperNonCriticalThreshold(), fr.getUpperCriticalThreshold(),
                    fr.getUpperNonRecoverableThreshold(), fr.getLowerNonRecoverableThreshold(),
                    fr.getLowerCriticalThreshold(), fr.getLowerNonCriticalThreshold(), fr.getNormalMinimum(),
                    fr.getNormalMaximum(), fr.getNominalReading());
